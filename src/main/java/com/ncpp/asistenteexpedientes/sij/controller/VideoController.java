...
import com.ncpp.asistenteexpedientes.sij.service.impl.VideoServiceImpl;
...
@RestController
@RequestMapping(Constants.VERSION_API+"/videos")
public class VideoController {
    ...
    public ResponseEntity<List<Archivo>> buscarTodo( @RequestParam String nUnico, @RequestParam int nIncidente ){
        ...
        try {
           lista=videoService.buscarTodo(nUnico,nIncidente);
        } catch (Exception e) {
    ...
    public ResponseEntity<Page<Fecha>> getFechas( @RequestParam  String nUnico, @RequestParam int nIncidente,Pageable page ){
        ...
        try {
           lista=videoService.getFechas(nUnico, nIncidente,page);
        } catch (Exception e) {
            System.out.println(e);			
            LogDony.write(this.getClass().getName()+" - ERROR: "+e);
            throw new InternalServerException();  
