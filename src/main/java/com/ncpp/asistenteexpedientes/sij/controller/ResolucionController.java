...
import com.ncpp.asistenteexpedientes.sij.service.impl.ResolucionServiceImpl;
...
@RestController
@RequestMapping(Constants.VERSION_API+"/resoluciones")
public class ResolucionController {
    ...
    public ResponseEntity<List<Archivo>> buscarTodo( @RequestParam String nUnico, @RequestParam int nIncidente ){
        ...
        try {
           lista=resolucionService.buscarTodo(nUnico, nIncidente);
        } catch (Exception e) {
    ...
    public ResponseEntity<Page<Fecha>> getFechas( @RequestParam String nUnico,@RequestParam int nIncidente, Pageable page ){
        ...
        try {
           lista=resolucionService.getFechas(nUnico,nIncidente,page);
        } catch (Exception e) {
            System.out.println(e);			
            LogDony.write(this.getClass().getName()+" - ERROR: "+e);
            throw new InternalServerException();  
