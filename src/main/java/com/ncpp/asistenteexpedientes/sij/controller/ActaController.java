...
import com.ncpp.asistenteexpedientes.sij.service.impl.ActaServiceImpl;
...
@RestController
@RequestMapping(Constants.VERSION_API+"/actas")
public class ActaController {

    ActaServiceImpl actaService;

    public ActaController(){
        actaService=new ActaServiceImpl();
    }
    ...
    public ResponseEntity<List<Archivo>> buscarTodo( @RequestParam String nUnico, @RequestParam int nIncidente ){
        ...
        try {
           lista=actaService.buscarTodo(nUnico, nIncidente);
        } catch (Exception e) {
    ...
    public ResponseEntity<Page<Fecha>> getFechas( @RequestParam  String nUnico, @RequestParam int nIncidente,Pageable page ){
        ...
        try {
           lista=actaService.getFechas(nUnico, nIncidente,page);
        } catch (Exception e) {
            System.out.println(e);			
            LogDony.write(this.getClass().getName()+" - ERROR: "+e);
            throw new InternalServerException();  
